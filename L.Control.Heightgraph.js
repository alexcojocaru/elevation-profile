  ...
  (function (factory, window) {
    ...
    L.Control.Heightgraph = L.Control.extend({
      ...
      onAdd: function onAdd(map) {
        ...
        var markerLabel = this._markerLabel = L.DomUtil.create("input", "heightgraph-marker-label", container);
      ...
      */
      ...
      _expand: function _expand() {
        ...
        if (!this._showState) {
          ...
          select(this._markerLabel).style("display", "block");
        } else {
          ...
          select(this._markerLabel).style("display", "none");
        }
      ...
      */
      ...
      _appendBackground: function _appendBackground() {
        var background = this._background = select(this._container)
              .select("svg")
              .select("g")
              .append("rect")
              ...
              .on("dblclick", this._mouseDblclickHandler.bind(this))
          ;
      ...
      */
      ...
      _mouseDblclickHandler: function() {
        // first, stop event propagation
        event.stopPropagation();
        
        const coords = mouse(this._svg.node());
        const xCoord = coords[0];
        const itemIndex = this._findItemForX(xCoord);

        if (itemIndex == -1) {
          return;
        }

        // if there is already a marker within 5px of xCoord, remove it
        const markerIndex = window.heightgraphMarkers.findIndex(marker => {
          const markerIndex = marker[0];
          const markerItem = this._areasFlattended[markerIndex];
          return Math.abs(markerItem.xDiagonalCoordinate - xCoord) < 5;
        });
        if (markerIndex > -1) {
          const crtItemIndex = window.heightgraphMarkers[markerIndex][0];
          // remove it from heightgraph
          this._svg.selectAll(".marker-group-" + crtItemIndex).remove();

          window.heightgraphMarkers.splice(markerIndex, 1);
        } else {
          // read the label, then reset the text field
          const label = this._markerLabel.value.trim();
          this._markerLabel.value = "";

          this.addPermanentMarker([itemIndex, label]);
        }
      },
    
      addPermanentMarker: function(marker) {
        
        const itemIndex = marker[0];
        const label = marker[1];

        const item = this._areasFlattended[itemIndex];

        const alt = this._defined(item) ? Math.round(item.altitude) : '-',
            dist = item.position.toFixed(2),
            xCoord = item.xDiagonalCoordinate;

        const markerGroupIndex = window.heightgraphMarkers.push(marker) - 1;

        const textLineHeight = 14;

        const markerGroup = this._svg
            .append("g")
            .attr("class", "marker-group marker-group-" + itemIndex);

        markerGroup
            .append("line")
            .attr("y1", this._y(this._elevationBounds.max))
            .attr("y2", this._y(this._elevationBounds.min))
            .attr('x1', xCoord)
            .attr('x2', xCoord);

        // use a group for all the text components, to help with relative positioning
        const markerTextGroup = markerGroup
            .append("g");

        const markerRect = markerTextGroup
            .append("rect")
            .attr("width", 0) // will calculate the width later
            .attr("height", textLineHeight);
        const markerText = markerTextGroup
            .append("text")
            .attr("x", 3)
            .attr("y", textLineHeight - 3)
            .text(label + (label ? " , " : "") + item.position.toFixed(1) + " k , " + alt + " m")
            // add helper class to help with the rect size calculations
            .classed("marker-group-text", true);

        const textWidth = this._dynamicBoxSize(".marker-group-text")[1];
        const rectWidth = textWidth + 6;
        // now that we are done with the rect size calculations, remove the helper classes
        markerText.classed("marker-group-text", false);
        markerRect.attr("width", rectWidth);

        // start the text positioning calculations; the text will be rotated at -90deg
        let textX = xCoord - textLineHeight;
        let textY = rectWidth;

        // if the box is overflowing to the left, move it below of the vertical line
        if (textX < 0) {
          textX = xCoord;
        }
        // if the current elevation is above 50% at this point, move the box to the bottom
        const minY = this._y(this._elevationBounds.min); // this is at the bottom of the graph
        const maxY = this._y(this._elevationBounds.max); // this is at the top of the graph
        const avgY = maxY + (minY - maxY) / 2; // top corner of the graph is (0, 0)
        if (this._y(alt) < avgY) {
          textY = minY;
        }

        markerTextGroup.attr("transform", "translate(" + textX + "," + textY + "),rotate(-90)");
      }
    });
